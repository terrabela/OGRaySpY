...
# 2017-10-02: mesmo que teste_une_pontos, mas este ser'a particularizado
#             para conexoes do tipo ridge em spec gama

def build_pts_for_ridges( npt, xmax, ymax):
    xs = []
    for y in range(ymax):
        xs_lev = [ np.random.randint(xmax) for ipt in range(npt)]
        xs_lev.sort()  # falta eliminar os duplicados!      
...
npt = 100
xs, ys = build_pts_for_ridges(npt,1000,20)
plt.plot( xs, ys, 'g.')
