...
class NuclideAnalysis:
    ...
    def create_matching_peaks_df(self, pks_df, en_toler, must_be_key_gamma=False):
    ...
    def nuclide_identif(self, nucl_iear1_df, pks_comprehensive_df):
        ...
        self.cross_df = pd.merge(pks_comprehensive_df, nucl_iear1_selctd_gamms_df, how='cross')
        self.cross_df["delta_en"] = self.cross_df.engy_pk_det - self.cross_df.energy

        en_toler_calib = 3.0
        matching_peaks_df = self.create_matching_peaks_df(self.cross_df, en_toler_calib, must_be_key_gamma=False)
    ...
    def final_nuclide_ident(self, pks_comprehensive_df, en_recalib):
        ...
        # self.cross_df = pd.merge(pks_comprehensive_df, nucl_iear1_selctd_gamms_df, how='cross')
        # self.cross_df["delta_en"] = self.cross_df.engy_pk_recalib - self.cross_df.energy
        # matching_peaks_df = self.create_matching_peaks_df(self.cross_df,, en_toler_ident)
        # print(matching_peaks_df)
